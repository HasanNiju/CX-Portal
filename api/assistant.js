...
const PRESETS_KEY = 'pathao:custom_presets';
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // builtInPresets: the client sends its hardcoded PRESETS array along
    // with the request, since that list lives in index.html, not Redis.
    const builtIn = Array.isArray(body.builtInPresets) ? body.builtInPresets : [];

    const redis = getRedis();
    const custom = (await redis.get(PRESETS_KEY)) || [];
    const all = [...builtIn, ...(Array.isArray(custom) ? custom : [])];
