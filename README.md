...
## Files added
- `lib/redis.js` — shared Upstash Redis client (kept outside `/api` on
  purpose — anything under `/api` becomes a public route on Vercel,
  and this is an internal helper, not an endpoint)
- `api/presets.js` — GET / POST `{ presets: [...] }`
- `api/hubs.js` — GET / POST `{ hubs: [...] }`
- `api/admin.js` — POST `{ action: 'verify' | 'set', password }`
- `api/diagnostics.js` — GET, reports Redis env var/connection status
...
2. Common causes:
   ...
   - **Different var names.** If you renamed the integration or it's a
     ...
     in `/api/diagnostics` will show what's actually there — `lib/redis.js`
   ...
   - A 404 on `/api/redis` or `/api/_redis` is expected and not a bug —
     that file lives in `/lib`, not `/api`, specifically so it's never
     an endpoint. The one URL to actually check is `/api/diagnostics`.
